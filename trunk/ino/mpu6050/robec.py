...
class robec:
	...
	def redis_subscribe(self):
		self.q = Queue.Queue()
		self.t = threading.Thread(target=redis_subscriber, args = (self.issuer(), self.q,))
...
#						print 'len=', len(self.recv_data)
	...
	def read_sockdata(self):
		while True:
			data = self.read_bin()
			if data:
				self.dbprint('Received BIN: %s' % data)
				self.process_bin(data)
			else:
...
#			self.dbprint('no JSON replied')
	...
	def run(self):
		...
		data = ''
		while True:
			cmd = self.check_rcommands()
			if cmd:
				self.dbprint("Command:%s" % cmd)
			ready2read, ready2write, in_error = select.select([self.sock], [], [], 1)
			for s in ready2read:
				if s == self.sock:
					self.spin()
					self.read_sockdata()
