...
#define INTER_PING_INTERVAL 50 // 50 ms.
...
class Sonar {
	protected:
		...
		int maxdistance;
	public:
		Sonar(uint8_t trigPin, uint8_t echoPin, int maxdist)
		{
			sonarTriggerPin = trigPin;
		...
		int ping()
		{
			...
			digitalWrite(sonarTriggerPin, LOW);
			int duration = pulseIn(sonarEchoPin, HIGH);
 
			//Calculate the distance (in cm) based on the speed of sound.
			int rs = duration/58.2;
			if(rs < 0 || rs > maxdistance) {
				rs = 0;
			}
			return rs;
		}
...
//  }
  ...
  Firmata.sendSysex(SYSEX_I2C_REPLY, available + 2, i2cRxData);
}
...
void sysexCallback(byte command, byte argc, byte *argv)
{
  ...
  case SONAR_CONFIG :
    ...
    {
      uint8_t sonarTriggerPin = argv[0] ;
      uint8_t sonarEchoPin = argv[1] ;
      ...
      sonars[numActiveSonars] = new Sonar(sonarTriggerPin, sonarEchoPin, max_distance) ;
      numActiveSonars++ ;
    }
    else {
...
void systemResetCallback()
{
  ...
  // stop pinging
  for (int i = 0; i < numActiveSonars; i++) {
  ...
  if (currentMillis - previousMillis > samplingInterval) {
    ...
    if ( pingLoopCounter++ > numLoops)
    {
      ...
      if (numActiveSonars)
      {
        ...
        int pingResult = sonars[nextSonar]->ping();
        ...
        Firmata.write(sonarPinNumbers[nextSonar]) ;
