...
#define AG_VAL_COUNT 7
union accel_t_gyro_union
{
	int buf[AG_VAL_COUNT];
	struct
	{
		int x_accel;
		int y_accel;
	...
	inline void transfer(uint16_t data[AG_VAL_COUNT]) {
		for(int i=0; i<AG_VAL_COUNT; i++) {
			buf[i] = data[i];
		}
	}
	inline void operator +=(accel_t_gyro_union val) {
		for(int i=0; i<AG_VAL_COUNT; i++) {
			buf[i] += val.buf[i];
		}
	}
...
class MPU6050:public ReServant
{
	private:
