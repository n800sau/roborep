...
with PiRGBArray(camera, size=tuple(conf["resolution"])) as rawCapture:
	imdir = conf["imdir"]
...
#			cv2.CHAIN_APPROX_SIMPLE)
		...
		numplace_found = False
		occupied = False

		dbprint('contours count: %d' % len(cnts))
		good_cnts = []
		...
		for c in cnts:
			...
			if area > conf["max_area"]:
				continue
			dbprint('Area: %s, Count: %d' % (area, len(c)))

			peri = cv2.arcLength(c, True)
			approx = cv2.approxPolyDP(c, 0.01 * peri, True)
			dbprint('contour len: %d' % len(approx))
#			if len(approx) == 4:
...
#			cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2)
		...
		if frame_counter == 0:
			...
			cv2.imwrite(os.path.join(imdir, fname_tmp), imutils.resize(frame, width=conf['file_width']))
			os.rename(os.path.join(imdir, fname_tmp), os.path.join(imdir, fname))

			if conf.get('ftp', None):
				ftp = FTP(conf['ftp']['host'], conf['ftp']['user'], conf['ftp']['password'])
				ftp.cwd(conf['ftp']['path'])
				outfname = 'frame@%s.jpg' % timestamp.strftime('%Y%m%d_%H%M%S')
				ftp.storbinary('STOR %s' % outfname, file(os.path.join(imdir, fname)))
				ftp.quit()

			fname = 'thresh.jpg'
			fname_tmp = 'thresh_tmp.jpg'
			cv2.imwrite(os.path.join(imdir, fname_tmp), imutils.resize(canny, width=conf['file_width']))
			os.rename(os.path.join(imdir, fname_tmp), os.path.join(imdir, fname))
			ft = t

		# check to see if the room is occupied
		if occupied and numplace_found:
			...
			if (timestamp - lastUploaded).seconds >= conf["min_upload_seconds"]:
				...
				if motionCounter >= conf["min_motion_frames"]:
					...
					cv2.imwrite(os.path.join(imdir, fname), imutils.resize(frame, width=conf['file_width']))
