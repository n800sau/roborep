...
// Which side to show
model=1; // [1:Pi 3 A+ (2018),2:Pi 3 B+ (2017),3:Pi 2 B+]
bothsides=3; // [0:Bottom Only,1:Both Sides (Preview Only),2:Top Only,3:Top Plate,4:Middle (Top - plate)]
...
top_left_design=5; // [0:None,1:Pi Logo,2:Custom Text,3:Open Source Logo,4:Custom Drawing,5:Air Vent,6:40x40 Fan,7:30x30 Fan,8:35x35 Fan,9:OctoPrint Tentacle,10:3D Print Logo,11:CNC Logo]
top_right_design=0; // [0:None,1:Pi Logo,2:Custom Text,3:Open Source Logo,4:Custom Drawing,5:Air Vent,6:40x40 Fan,7:30x30 Fan,8:35x35 Fan,9:OctoPrint Tentacle,10:3D Print Logo,11:CNC Logo]
...
bottom_offset_y=6;
bottom_design_text="####";
bottom_design_scale=0.85; // [0:0.1:5]
...
module full_case() {
    ...
    union()
    {
      difference() {
        ...
        if(!block_power)
        {
          ...
          if(support_power)
            translate([-ooff-14,th-18,0]) microusb_support();
        }
      }
      port_holes();
    }
        ...
        translate([0,0,td-(thinmode?6:2)]) difference() {
          ...
          if(top_right_design)
            translate([0,0,-.01]) difference() {
              ...
              if(bothsides==3&&(top_right_design<5||top_right_design>8))
                linear_extrude(top_plate_design_base,convexity=3) offset(oof) square2([tw,th],1);
            }
          if(top_left_design)
            translate([0,0,-.01]) difference() {
          ...
          if(cambox_attachment) {
            for(pos=[
                ...
                translate([0,0,-.01]) linear_extrude(2.02,convexity=3)
                  translate([tw-pos[0],th-pos[1]]) circle(d=3.6);
            }
          }
        }
        port_holes();
  ...
  if(!block_sd_card)
  translate([20,th,-.01]) {
    translate([1.4,0.85-(model<3?4:0)]) linear_extrude(7.3) square2([13.5,5],0.5);
...
{
  translate([-6,0]) difference() {
    cube([21,15,10.3]);
    translate([-.01,7.5,9]) {
      rotate([0,90,0]) cylinder(d=4,h=6,$fn=40);
      translate([0,-1.75]) cube([6,3.5,5]);
    }
    translate([5.98,1.5,6]) {
      cube([15.03,12,5]);
    }
  }
}
module port_holes()
{
  if(!block_usb)
  {
    if(model==1)
      ...
      for(x=[22,39.5]) translate([x,-ooff-.1,8]) {
        cube([14.5,18,16]);
...
module fan(size=40)
{
  ...
  mirrorx() mirrory() translate([hoff/2,hoff/2]) circle(d=3.5,$fn=20);
}
