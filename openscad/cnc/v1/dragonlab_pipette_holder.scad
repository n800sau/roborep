...
module pipette_bottom_holder_top() {
	translate([0, pipette_center_off_y-attach_sz_y/2-gantry_sz_y/2, holder_top_sz_z/2]) {
		difference() {
//		union() {
			union() {
				translate([0, pipette_center_off_y, 0]) {
					at_h = attach_sz_z/2;
					for(x=[-1,1]) {
						translate([x*(holder_sz_x+wall+1)/2, -pipette_center_off_y-5, at_h/2-holder_top_sz_z/2]) {
							difference() {
...
//									cube([20, holder_sz_y/2+10, at_h], center=true);
			...
			translate([0, -default_attach_extra_sz_y-(pipette_center_off_y-attach_sz_y/2-gantry_sz_y/2), -holder_top_sz_z/2]) {
				attachment_holes(d=hole_d_through);
			}
			translate([0, pipette_center_off_y, 0]) {
				cylinder(d=widest_level_d+2*pipette_gap, h=holder_top_sz_z, center=true);
			}
			translate([0, -widest_level_d/2-20, 0]) {
