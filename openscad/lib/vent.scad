module vent(x_size=30, x_count=9, z_size=30, z_count=9, y_size=2, center=true, negate=false) {
  if(negate) {
    difference() {
      cube([x_size, y_size, z_size], center=center);
      vent(x_size, x_count, z_size, z_count, y_size, center);
    }
  } else {
