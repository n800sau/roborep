...
module tube_block_top() {
	translate([0, 0, wall+tube_sz+wall/2]) {
		difference() {
      translate([(-wall-4-wall/2)/2, 0, 0]) {
        cube([tube_len+wall+4+wall/2, tube_sz+2*wall, wall], center=true);
      }
			// side wall ear holes
			for(ysgn=[-1,1]) {
				for(xsgn=[-1,1]) {
...
module slit_plate() {
	translate([(-tube_len-wall-4-gap)/2, gap/2, wall+(tube_sz-gap)/2]) {
		difference() {
			cube([4-gap-0.5, tube_sz-gap-0.5, tube_sz-gap-0.5], center=true);
			cube([4-gap, 0.5, 30], center=true);
		}
	}
}
...
module camera_block() {
    ...
    for(xsgn=[0,-1]) {
      for(ysgn=[-1]) {
				translate([-4+xsgn*(tube_sz-10), 90+ysgn*-18, tube_sz+wall]) {
          rotate([0, 0, xsgn*180]) {
            color("green") ear(0, 10, cone=true);
          }
        }
    ...
    difference() {
      ...
      translate([0, -tube_sz/2-wall, -wall/2]) {
        ...
        translate([60, 0, 0]) {
          for(xsgn=[0,-1]) {
            for(ysgn=[-1]) {
              translate([-4+xsgn*(tube_sz-10), 90+ysgn*-18, 0]) {
                cylinder(d=hole_d_through, h=20,center=true);
              }
            }
          }
...
//         color("blue") grating_insert();
      ...
      translate([0, 0, -5]) {
//        oval_washer(ext_d=25, d=4, h=1, dist=10);
      }
    }
  }
