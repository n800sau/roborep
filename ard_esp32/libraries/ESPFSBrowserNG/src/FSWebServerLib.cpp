...
void AsyncFSWebServer::sendTimeData() {
	String data = "{";
	data += "\"time\":\"" + (String)NTP.getTimeStr() + "\",";
	data += "\"date\":\"" + (String)NTP.getDateStr() + "\",";
	data += "\"lastSync\":\"" + (String)NTP.getTimeDateString(NTP.getLastNTPSync()) + "\",";
	data += "\"uptime\":\"" + (String)NTP.getUptimeString() + "\"";
	data += "}\r\n";
	DEBUGLOG(data.c_str());
...
//		NTP.begin(_config.ntpServerName, _config.timezone / 10, _config.daylight);
		NTP.begin(_config.ntpServerName.c_str());
		NTP.setInterval(15, _config.updateNTPTimeEvery * 60);
	}

	// Register wifi Event to control connection LED
    WiFi.onEvent([this](WiFiEvent_t event, WiFiEventInfo_t info) {
		this->onWiFiConnected(info);
	}, WiFiEvent_t::SYSTEM_EVENT_STA_CONNECTED);

    WiFi.onEvent([this](WiFiEvent_t event, WiFiEventInfo_t info) {
		this->onWiFiDisconnected(info);
	}, WiFiEvent_t::SYSTEM_EVENT_STA_DISCONNECTED);

    WiFi.onEvent([this](WiFiEvent_t event, WiFiEventInfo_t info) {
		this->onWiFiConnectedGotIP(info);
...
void AsyncFSWebServer::configureWifiAP() {
	...
	//WiFi.disconnect();
	WiFi.mode(WIFI_AP);
	String APname = _apConfig.APssid + "_" + getMacAddress();
	if (_httpAuth.auth) {
...
void AsyncFSWebServer::onWiFiConnected(WiFiEventInfo_t info) {
...
void AsyncFSWebServer::onWiFiConnectedGotIP(WiFiEventInfo_t info) {
...
void AsyncFSWebServer::onWiFiDisconnected(WiFiEventInfo_t info) {
	...
	if (request->args() > 0)  // Save Settings
	{
		...
		for (uint8_t i = 0; i < request->args(); i++) {
			if (request->argName(i) == "ntpserver") {
				...
				NTP.setNtpServerName(_config.ntpServerName.c_str());
				continue;
			}
...
//				NTP.setTimeZone(_config.timezone / 10);
				NTP.setTimeZone(request->arg(i).c_str());
				continue;
			}
