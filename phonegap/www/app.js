	...
	function startScan()
	{
		...
		evothings.ble.startScan(
			...
			function(device)
			{
				...
				if(device.name.startsWith('AM2320')) {
					// Update device data.
...
//			return mapDeviceRSSI(device1.rssi) < mapDeviceRSSI(device2.rssi)
	...
	function displayDevices()
	{
		...
		for (var i = 0; i < sortedList.length; ++i)
		{
			...
			var htmlDevice =
				...
				+	htmlDeviceManufacturerData(device)
				+ '</p>'
			html += htmlDevice
		}
	...
	function htmlDeviceManufacturerData(device)
	{
		var rs = '';
		if(device.advertisementData.kCBAdvDataManufacturerData) {
			var d = atob(device.advertisementData.kCBAdvDataManufacturerData);
//			rs = 'Data: ' + device.advertisementData.kCBAdvDataManufacturerData + '<br/>';
//			for(var i=0; i<d.length; i++) {
//				rs += ' 0x' + d.charCodeAt(i).toString(16);
//			}
//			rs += '<br/>';
			var v = (d.charCodeAt(2) + (d.charCodeAt(3) << 8) + (d.charCodeAt(4) << 16) + (d.charCodeAt(5) << 24))/ 1000.;
			if(device.name.endsWith('_H')) {
				rs += 'Humidity: ' + v + '&percnt;';
			} else if(device.name.endsWith('_T')) {
				rs += 'Temperature: ' + v + '&deg;C';
			} else if(device.name.endsWith('_V')) {
				rs += 'Battery: ' + v + 'v';
			}
			rs += '<br/>';
		} else {
...
//		return JSON.stringify(device.advertisementData, null, '<br>');
//		return device.advertisementData.kCBAdvDataManufacturerData ? 'Data:' + device.advertisementData.kCBAdvDataManufacturerData.toString('hex') + '<br/>' : '';
		return rs;
	}
