...
namespace grideye {
	...
	static SoftwareSerial Serial(A5, 99); // RX, TX
	...
	static unsigned serial_readline()
	{
		unsigned pos = 0;
		...
		if(rs) {
			...
			for(int r=0; r<8; r++) {
				for(int c=0; c<8; c++) {
					...
					terms[c][7-r] = linebuf.t.terms[r][c] * multiplier_tarr;
					test_touch();
//					Serial.print(int(terms[c][7-r]), DEC);
//					Serial.print(" ");
				}
//				Serial.println();
			}
