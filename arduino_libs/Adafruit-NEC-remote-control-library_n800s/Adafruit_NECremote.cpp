...
int16_t Adafruit_NECremote::listen(int16_t maxwaitmillis) {
	uint16_t p1, p2;

	p1 = p2 = 0;
	maxwaiting = maxwaitmillis;
	maxwaiting *= 1000;	// to microseconds

	if (maxwaitmillis) timing = true;
	timedout = false;
...
//	Serial.println(data[3], HEX);
	...
	if (((data[0] == (~data[1] & 0xFF)) && (data[2] == (~data[3] & 0xFF))) ||
			((data[0] == 0) && (data[1] == 0xBF) && (data[2] == (~data[3] & 0xFF))))
	{
		return ((data[0] << 8) & 0x7F00) + data[2];
	}
