...
void loop() {
	...
	if(test_count < N_TESTS) {
		...
		for(i=0; i<6; i++) {
			int32_t off = vcoll[i] / N_TESTS;
			if(off > 10) {
				corrs[i]--;
			} else if (off < 10) {
				corrs[i]++;
			}
			int16_t corr = corrs[i];
			Serial.print(i);
			Serial.print(": off=");
			Serial.print(off);
			Serial.print(" corr=");
			...
			switch(i) {
				case AX:
					accelgyro.setXAccelOffset(corr);
					break;
				case AY:
					accelgyro.setYAccelOffset(corr);
					break;
				case AZ:
					accelgyro.setZAccelOffset(corr);
					break;
				case GX:
					accelgyro.setXGyroOffset(corr);
					break;
				case GY:
					accelgyro.setYGyroOffset(corr);
					break;
				case GZ:
					accelgyro.setZGyroOffset(corr);
					break;
			}
			vcoll[i] = 0;
		}
