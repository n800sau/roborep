...
def predict_location(m, yolo_losses, input_dataset, batch_size=1, gpu_id=-1):
	...
	for batch_idx,batch in enumerate(input_dataloader):
		...
		for idx, detections in enumerate(batch_detections):
			...
			input_image = batch['image'][idx].cpu().transpose(0, 2)
...
if __name__ == "__main__":
	...
	input_dataset = PredictImageDataset(input_dir=C.PREDICT_INPUT_DIR, img_size=(TRAINING_PARAMS["img_w"], TRAINING_PARAMS["img_h"]))
	# YOLO loss with 3 scales
	yolo_losses = []
	for i in range(3):
		yolo_losses.append(YOLOLoss(TRAINING_PARAMS["anchors"][i], C.NUM_CLASSES, (TRAINING_PARAMS["img_w"], TRAINING_PARAMS["img_h"])))
	for bname,idx,img_arr in predict_location(load_model(), yolo_losses, input_dataset, batch_size=C.BATCH_SIZE, gpu_id=C.GPU_ID):
