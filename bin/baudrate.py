...
class Baudrate:
	...
	def __init__(self, port=None, threshold=MIN_CHAR_COUNT, timeout=READ_TIMEOUT, name=None, auto=True, verbose=False, init_string=None):
		self.port = port
	...
	def NextBaudrate(self, updn):
		...
		if self.init_string:
			self.serial.write(self.init_string)
		self.serial.flush()
...
if __name__ == '__main__':
	...
	def main():
		...
		baud = Baudrate(port, threshold=threshold,
						timeout=timeout, name=name, verbose=verbose, auto=auto, init_string='+++')
