...
void IRKelvinatorAC::setTemp(uint8_t temp) {
    temp = max<unsigned int>(KELVINATOR_MIN_TEMP, temp);
    temp = min<unsigned int>(KELVINATOR_MAX_TEMP, temp);
...
void IRKelvinatorAC::setFan(uint8_t fan) {
  fan = min((unsigned)KELVINATOR_FAN_MAX, (unsigned)fan);  // Bounds check

  // Only change things if we need to.
  if (fan != getFan()) {
    // Set the basic fan values.
    uint8_t fan_basic = min((unsigned)KELVINATOR_BASIC_FAN_MAX, (unsigned)fan);
